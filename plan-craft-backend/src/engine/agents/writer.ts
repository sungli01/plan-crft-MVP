...
13. **통계 데이터 표 필수**: 각 섹션에 최소 1개의 관련 통계 데이터를 Markdown 표(table)로 포함하라.
    - 시장 데이터, 성능 비교, 비용 분석, 트렌드 수치 등 섹션 주제에 맞는 정량 데이터를 표로 정리

14. **인라인 인용 필수**: 본문에서 주장, 통계, 데이터를 인용할 때 반드시 [1], [2] 형태의 인라인 인용 번호를 달아라.
    - 리서치 컨텍스트에 포함된 논문/참고문헌을 적극 활용하여 인용하라
    - 출처 없는 주장은 최소화하고, 가능한 모든 수치에 인용을 붙여라

15. **참고문헌(References) 섹션 필수**: 문서의 마지막 섹션으로 반드시 '참고문헌(References)' 섹션을 작성하라.
    - 형식: [번호] 저자, 제목, 출처, 년도, URL(있는 경우)
    - 본문에서 인용한 모든 출처를 빠짐없이 포함

16. **전문용어 설명**: 전문용어가 문서에서 처음 등장할 때 괄호 안에 한국어 설명을 붙여라.
    - 예: LiDAR(Light Detection and Ranging, 레이저 기반 거리 측정 기술)
    - 예: CAGR(Compound Annual Growth Rate, 연평균 복합 성장률)
    - 예: TAM(Total Addressable Market, 전체 시장 규모)
    - 약어, 영문 기술 용어, 업계 전문 용어 모두 해당
...
export class WriterAgent {
  ...
  async writeSection(section: SectionInfo, projectInfo: { title: string; idea?: string; categoryId?: string }, context: WriteContext = {}): Promise<WriteSectionResult> {
    ...
    let slideContextLine = '';
    if (context.slideContext) {
      const sc = context.slideContext;
      slideContextLine = `\n\n[프레젠테이션 슬라이드 컨텍스트 — 반드시 반영]
슬라이드 ${sc.pageNumber}: "${sc.title}"
핵심 메시지: ${sc.keyMessage}
${sc.bullets ? `주요 포인트:\n- ${sc.bullets.join('\n- ')}` : ''}
${sc.kpiValues ? `핵심 수치: ${sc.kpiValues.join(', ')}` : ''}
→ 이 슬라이드의 핵심 메시지를 본문에 자연스럽게 확장하세요. 슬라이드에 언급된 수치/KPI를 반드시 포함하세요.`;
    }
...
${researchWarning}${slideContextLine}
