...
export default function Generate() {
  ...
  const { startGenerate, download, status: genStatus } = useGenerate({
    onComplete: (result) => {
      setGeneratedDoc({
        title: generatedDoc?.title || `${category?.label} 결과물`,
        content: result || "문서 생성이 완료되었습니다.",
      });
      setProgress(100);
  ...
  useEffect(() => {
    if (genStatus) {
      if (genStatus.progress !== undefined) {
        setProgress(genStatus.progress);
        const stepIndex = Math.min(
          Math.floor((genStatus.progress / 100) * generationSteps.length),
          generationSteps.length - 1
        );
        setCurrentStep(stepIndex);
      }
    }
  }, [genStatus, generationSteps.length]);
